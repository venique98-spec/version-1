...
# - No Group A/B concepts
# - No special "Age 1 classroom #5 requires a Director" rule (REMOVED)
...
# ──────────────────────────────────────────────────────────────────────────────
# Slot plan — every slot becomes its own row. No special group handling.
# ──────────────────────────────────────────────────────────────────────────────
def build_slot_plan():
    return {
        # Age 1
        "Age 1 leader": 1,
        "Age 1 classroom": 5,        # (No special gate on #5 anymore)
        "Age 1 nappies": 1,
        "Age 1 bags girls": 1,
        "Age 1 bags boys": 1,
        # Age 2
        ...
        for d in service_dates:  # try earliest dates first
            ...
            for slot_row in slot_rows_sorted:
                ...
                if base_role not in p1_roles_by_person[p]:
                    continue
                # respect eligibility (0 = not allowed) — P1 comes from pr==1 but check anyway
            ...
            if got_one:
                break
        # if not got_one, we'll report unmet P1 later
