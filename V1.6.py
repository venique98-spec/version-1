...
# - Slot plan built dynamically from all roles in Positions CSV headers (with exclusions)
...
def dedupe_latest_by_name(df: pd.DataFrame, name_col: str) -> pd.DataFrame:
    ...
    if "Timestamp" in df.columns:
        ...
        return latest
    # Fallback: keep the last occurrence per person in file order
...
DEFAULT_CAPS = {
    ...
    # Common extras (entrance greeter REMOVED)
    "info": 4,
    "ukids setup": 4,
    "outside assistant": 2,
    ...
    "brooklyn babies serving girl": 3,   # ← increased from 2 to 3
    "brooklyn preschool leader": 1,
    "brooklyn preschool": 3,             # + leader = total 4
}
...
def build_slot_plan_dynamic(all_role_headers):
    ...
    for role in all_role_headers:
        if normalize(role) in excluded:
            continue  # skip excluded roles entirely
...
def compute_p1_roles_by_person(long_df, allowed_roles_set):
    """Return {person: set of base roles with priority == 1}, filtered to allowed roles."""
    p1 = defaultdict(set)
...
def main_pass_schedule(long_df, availability, service_dates, role_codes, all_role_headers):
    # Apply exclusions to the header list
    excluded = {normalize(x) for x in EXCLUDED_ROLES}
    allowed_headers = [h for h in all_role_headers if normalize(h) not in excluded]

    # Build slot plan dynamically from allowed headers (ensures Brooklyn etc. show up)
    slot_plan = build_slot_plan_dynamic(allowed_headers)
    ...
    p1_roles_by_person = compute_p1_roles_by_person(long_df, allowed_roles_set=set(allowed_headers))
...
if st.button("Generate Schedule", type="primary"):
    ...
    # Apply exclusions here too (belt and braces)
    excluded = {normalize(x) for x in EXCLUDED_ROLES}
    role_cols = [c for c in role_cols_all if normalize(c) not in excluded]

    if not role_cols:
        st.error("No usable role columns detected in the positions CSV (from third column onwards).")
        st.stop()
