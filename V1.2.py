...
# Plus extra rows (Group A/B) with special rules (now base-role named).
# ──────────────────────────────────────────────────────────────────────────────
...
def build_slot_plan():
    return {
        ...
        # Group A (base roles with capacities)
        "Info": 4,
        "uKids setup": 4,
        # Group B (base roles with capacities)
        "Outside assistant": 2,
        "Helping ninja and check-in leader": 1,  # leaders only (gate below)
        "Helping ninja": 2,
        "uKids hall": 4,
    }
...
def served_in_priority_one(assignments_by_cell, priority_lut, slot_to_role):
    ...
    Example: "Age 2 classroom #3" → base role "Age 2 classroom"
    """
    served = set()
...
# ──────────────────────────────────────────────────────────────────────────────
# Core scheduling (main pass)
# ──────────────────────────────────────────────────────────────────────────────
def build_eligibility(long_df: pd.DataFrame):
    # set of roles per person (roles with pr>=1)
...
def role_allowed_for_person(eligibility, person, base_role):
    """Check if base_role is allowed for person based on positions (pr>=1)."""
...
def main_pass_schedule(long_df, availability, service_dates, role_codes):
        ...
        for slot_row in slot_rows_sorted:
            ...
            for p in people:
                ...
                if not availability.get(p, {}).get(d, False):
                    continue
                # eligible for this base role? (respect 0 = not allowed)
    ...
    # return eligibility and people so extra passes can enforce 0s too
    return schedule_cells, assign_count, slot_rows, slot_to_role, eligibility, people

# ──────────────────────────────────────────────────────────────────────────────
# Post Passes
#   A) Allow +1 cap ONLY in Group A rows (Info / uKids setup), respecting eligibility
#   B) Fill Group B rows only with people who served in a #1 role (no cap raise), respecting eligibility
# ──────────────────────────────────────────────────────────────────────────────
def assign_extra_group_a(service_dates, schedule_cells, assign_count, availability, role_codes, slot_to_role, eligibility, people):
    for d in service_dates:
        assigned_today = {nm for (rn, dd), names in schedule_cells.items() if dd == d for nm in names}
        for row_name, dd in [k for k in schedule_cells.keys() if k[1] == d]:
            base_role = slot_to_role.get(row_name, row_name)
            # only apply to Group A base roles
            if normalize(base_role) not in {normalize("Info"), normalize("uKids setup")}:
                continue
            ...
            # candidates: available, not assigned today, under (base+1), and ALLOWED for base_role
            cands = []
            for p in people:
                if not availability.get(p, {}).get(d, False):
                    continue
                ...
                if assign_count.get(p, 0) >= base_cap + 1:
                    continue
                if not role_allowed_for_person(eligibility, p, base_role):
                    continue
                cands.append(p)
...
def assign_extra_group_b(service_dates, schedule_cells, assign_count, availability, role_codes, served_p1_people, slot_to_role, eligibility, people):
    for d in service_dates:
        assigned_today = {nm for (rn, dd), names in schedule_cells.items() if dd == d for nm in names}
        for row_name, dd in [k for k in schedule_cells.keys() if k[1] == d]:
            base_role = slot_to_role.get(row_name, row_name)
            # only apply to Group B base roles
            if normalize(base_role) not in {
                normalize("Outside assistant"),
                normalize("Helping ninja and check-in leader"),
                normalize("Helping ninja"),
                normalize("uKids hall"),
            }:
                continue
            ...
            for p in people:
                ...
                flags = role_codes.get(p, {})
                base_cap = base_max_for_person(flags)
                if assign_count.get(p, 0) >= base_cap:
                    continue
                ...
                if normalize(base_role) == normalize("Helping ninja and check-in leader") and not is_ukids_leader(flags):
                    continue
                # respect eligibility (0 = not allowed)
                if not role_allowed_for_person(eligibility, p, base_role):
                    continue
                cands.append(p)
...
def build_person_assignment_details(schedule_cells):
    """
    Returns dict: person -> 'YYYY-MM-DD — Slot; YYYY-MM-DD — Slot; ...'
    Sorted by date for readability.
    """
...
st.caption("• Positions CSV: first col = volunteer names; second col = role codes (e.g., D/BL/PL/EL/SL), other cols = roles with values 0–5 (0 = not eligible).")
st.caption("• Responses CSV: includes a name column and columns like 'Are you available 7 September?'.")
...
if st.button("Generate Schedule", type="primary"):
    ...
    # MAIN PASS
    schedule_cells, assign_count, slot_rows, slot_to_role, eligibility, people = main_pass_schedule(
    ...
    # PASS A: allow +1 cap (non-D to 3, D to 2) but ONLY for Group A rows — enforce eligibility
    assign_extra_group_a(service_dates, schedule_cells, assign_count, availability, role_codes, slot_to_role, eligibility, people)

    # PASS B: fill Group B only if already served in a #1 role (no cap raise) — enforce eligibility
    assign_extra_group_b(service_dates, schedule_cells, assign_count, availability, role_codes, served_p1_people, slot_to_role, eligibility, people)
