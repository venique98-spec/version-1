...
def dedupe_latest_by_name(df: pd.DataFrame, name_col: str) -> pd.DataFrame:
    ...
    if "Timestamp" in df.columns:
        ...
        # keep the last (most recent) per person
...
DEFAULT_CAPS = {
    ...
    "helping ninja": 2,
    "ukids hall": 4,
    # Brooklyn (default to 1 if not listed here)
    "brooklyn runner": 1,
    "brooklyn babies leader": 1,
    "brooklyn babies serving girl": 1,
    "brooklyn preschool leader": 1,
    "brooklyn preschool": 1,
}

def build_slot_plan_dynamic(all_role_headers):
    slot_plan = {}
    # Normalize caps map for robust matching
...
def main_pass_schedule(long_df, availability, service_dates, role_codes, all_role_headers):
    # Build slot plan dynamically from all headers (ensures Brooklyn etc. show up)
        ...
        for d in service_dates:  # try earliest dates first
            ...
            for slot_row in slot_rows_sorted:
                ...
                if normalize(base_role) == normalize("Helping ninja and check-in leader") and not is_ukids_leader(flags):
                    continue
                # slot available? (ENFORCE single person per slot)
